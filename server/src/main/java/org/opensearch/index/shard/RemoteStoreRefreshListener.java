...
public class RemoteStoreRefreshListener implements ReferenceManager.RefreshListener {
    ...
    private static final String INTERMEDIATE_SEGMENTINFOS_FILENAME = "intermediate_segments_N";
    ...
    public void afterRefresh(boolean didRefresh) throws IOException {
        if (didRefresh) {
            ...
            if (uploadSuccess.get()) {
                IndexOutput indexOutput = storeDirectory.createOutput(INTERMEDIATE_SEGMENTINFOS_FILENAME, IOContext.DEFAULT);
                SegmentInfos segmentInfos = new SegmentInfos(Version.LATEST.major);
                segmentInfos.write(indexOutput);
                indexOutput.close();
                storeDirectory.sync(Collections.singleton(INTERMEDIATE_SEGMENTINFOS_FILENAME));
                remoteDirectory.copyFrom(storeDirectory, INTERMEDIATE_SEGMENTINFOS_FILENAME, INTERMEDIATE_SEGMENTINFOS_FILENAME, IOContext.DEFAULT);
