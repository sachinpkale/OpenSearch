...
public class RemoteStorePinnedTimestampService implements Closeable {
    ...
    private Tuple<String, Long> getPinningEntityTimestampFromBlobName(String blobName) {
    ...
    public static Set<Long> getPinningEntityTimestampMap(String pinningEntityPrefix) {
        Set<String> matchingPinningEntities = pinningEntityTimestampMap.v2().keySet().stream().filter(entity -> entity.startsWith(pinningEntityPrefix + PINNED_TIMESTAMPS_FILENAME_SEPARATOR)).collect(Collectors.toSet());
